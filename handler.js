...
user.commandCount = 1;
               ...
               if (plugin.rowner && plugin.owner && !(isROwner || isOwner)) { // Both Owner
                ...
                if (!isPrems && plugin.money && global.db.data.users[m.sender].money < plugin.money * 1) {
                    this.reply(m.chat, `𝙉𝙊 𝙏𝙄𝙀𝙉𝙀 𝙒𝙄𝙏𝙏𝙔𝘾𝙊𝙄𝙉𝙎`, m)
                    continue     
		}
		    ...
		    continue // If the level has not been reached
                ...
                try {
                 ...
                 if (m.money)
                        m.reply(+m.money + ' 𝙒𝙄𝙏𝙏𝙔𝘾𝙊𝙄𝙉𝙎 𝙐𝙎𝘼𝘿𝙊(𝙎)')
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '𝗦𝘂𝗽𝗲𝗿 𝙒𝙞𝙩𝙩𝙮-𝘽𝙤𝙩') :
...
"thumbnail": apii.data, 
"title": [wm, '𝗦𝘂𝗽𝗲𝗿 ' + wt + ' ', '🌟 instagram.com/gabiacosta06'].getRandom(),
"containsAutoReply": true,
"mediaType": 1, 
sourceUrl: [md, nna, yt, ig, fb].getRandom()}}}, { quoted: fkontak2 }) 
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, '𝗦𝘂𝗽𝗲𝗿 ' + wt + ' ', '🌟 instagram.com/gabiacosa06'].getRandom(), thumbnail: wittyImg.getRandom(), sourceUrl: [md, nna, yt, ig, fb].getRandom() }}}}, tg)
